// Inicialización segura de variables globales necesarias para el dashboard
if (typeof weekOffset === 'undefined') window.weekOffset = 0;
if (typeof MONTH_NAMES === 'undefined') window.MONTH_NAMES = ['Enero', 'Febrero', 'Marzo', 'Abril', 'Mayo', 'Junio', 'Julio', 'Agosto', 'Septiembre', 'Octubre', 'Noviembre', 'Diciembre'];
if (typeof DAY_NAMES === 'undefined') window.DAY_NAMES = ['lun.', 'mar.', 'mié.', 'jue.', 'vie.', 'sáb.', 'dom.'];
if (typeof HOUR_START === 'undefined') window.HOUR_START = 8;
if (typeof HOUR_END === 'undefined') window.HOUR_END = 21;
if (typeof SERVICE_COLORS === 'undefined') window.SERVICE_COLORS = { otro: { bg: '#e2e8f0', border: '#94a3b8', text: '#222' } };
if (typeof STATUS_LABELS === 'undefined') window.STATUS_LABELS = { pendiente: '⏳', confirmada: '✅', cancelada: '❌' };
// Dropdown de vista de calendario personalizado y lógica de cambio de vista
let calendarViewMode = 'semana7';
    ...
    if (viewBtn && viewMenu) {
        ...
        viewMenu.querySelectorAll('.cal-dropdown-item').forEach(item => {
            item.addEventListener('click', () => {
                ...
                viewMenu.style.display = 'none';
                calendarViewMode = item.dataset.view;
                renderCalendar();
            });
        });
    }
...
async function renderCalendar() {
    ...
    // Determinar días a mostrar según vista
    let days = [];
    if (calendarViewMode.startsWith('semana7')) {
        for (let i = 0; i < 7; i++) {
            const d = new Date(monday);
            d.setDate(monday.getDate() + i);
            days.push(d);
        }
    } else if (calendarViewMode.startsWith('semana5')) {
        for (let i = 0; i < 5; i++) {
            const d = new Date(monday);
            d.setDate(monday.getDate() + i);
            days.push(d);
        }
    } else if (calendarViewMode.startsWith('dia')) {
        days = [now];
    } else {
        // Por defecto semana 7 días
        for (let i = 0; i < 7; i++) {
            const d = new Date(monday);
            d.setDate(monday.getDate() + i);
            days.push(d);
    ...
    const fechaFin = days[days.length - 1].toISOString().slice(0, 10);
    let citasPorDia = {};
    try {
    ...
    let html = '';
    if (calendarViewMode.endsWith('listado')) {
        // Vista listado
        days.forEach((d, i) => {
            const fecha = d.toISOString().slice(0, 10);
            const isToday = isSameDay(d, now);
            html += `<div class="cal-listado-dia${isToday ? ' cal-today' : ''}">
                    <div class="cal-listado-header">
                        <i class="fa-regular fa-calendar"></i> <strong>${DAY_NAMES[i].toUpperCase()} ${d.getDate()}</strong>
                    </div>`;
            const citas = citasPorDia[fecha] || [];
            if (citas.length === 0) {
                html += '<div class="cal-listado-vacio">Sin citas</div>';
            } else {
                citas.forEach(cita => {
                    const colors = SERVICE_COLORS[cita.servicio] || SERVICE_COLORS.otro;
                    const statusIcon = STATUS_LABELS[cita.estado] || '';
                    html += `<div class="cal-listado-cita" style="background:${colors.bg};border-left:3px solid ${colors.border};color:${colors.text};margin-bottom:6px;padding:8px 12px;border-radius:6px;">
                            <strong>${cita.hora}</strong> ${statusIcon} — <b>${cita.paciente_nombre}</b> <span style="opacity:.7">${cita.servicio}</span>
                        </div>`;
                });
            }
            html += '</div>';
